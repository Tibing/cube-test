...
class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.apollo.query({
      ...
      this.dashboardItems.next((result?.data?.dashboardItems || []).map((item, index) => {
        const layout = item.layout?.length > 2 ? JSON.parse(item.layout) : null;
  ...
  deleteItem(id: number) {
    this.apollo.mutate({
      ...
      this.dashboardItems.next(this.dashboardItems.value.filter(({
        id
      }) => id != result?.data?.deleteDashboardItem.id));
    });
  }
