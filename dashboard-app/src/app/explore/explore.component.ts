...
import { BuilderMeta, CubejsClient, Query, QueryBuilderService } from '@cubejs-client/ngx';
...
class ExploreComponent implements OnInit, OnDestroy {
  ...
  constructor(public cubejsClient: CubejsClient, public queryBuilder: QueryBuilderService, public dialog: MatDialog, private route: ActivatedRoute) {
    queryBuilder.setCubejsClient(cubejsClient);
    this.chartTypeMap = this.chartTypeToIcon.reduce((memo, {
      chartType,
      icon
    }) => ({ ...memo,
      [chartType]: icon
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (params.query) {
        this.queryBuilder.deserialize({
          ...
          pivotConfig: params.pivotConfig && JSON.parse(params.pivotConfig) || null,
