...
import { isQueryPresent, PivotConfig as TPivotConfig, Query, ResultSet } from '@cubejs-client/core';
...
class QueryRendererComponent implements OnInit {
  ...
  constructor(private cubejsClient: CubejsClient, private snakBar: MatSnackBar, private dialog: MatDialog) {}

  ngOnInit() {
    combineLatest([this.cubeQuery$.pipe(switchMap(cubeQuery => {
      if (!isQueryPresent(cubeQuery || {})) {
        return of(null);
      }

      this.data.cubeQuery = cubeQuery;
      return this.cubejsClient.load(cubeQuery).pipe(catchError(error => {
        this.snakBar.open(error.message || 'Request error', null, {
          duration: 2000
        });
        return of(null);
      }));
    })), this.pivotConfig$, this.chartType$]).subscribe(([resultSet, pivotConfig, chartType]: [ResultSet, TPivotConfig, TChartType]) => {
